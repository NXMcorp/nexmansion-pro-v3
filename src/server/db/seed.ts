...
function upsertCountry(db: Database.Database, id: string, name: string, slug: string, iso: string) {
...
function upsertDestination(db: Database.Database, id: string, name: string, slug: string, description: string, img: string, lat: number, lng: number, countryId: string) {
...
function upsertRegion(db: Database.Database, id: string, name: string, slug: string, destId: string) {
...
function upsertCollection(db: Database.Database, id: string, title: string, slug: string, subtitle: string, desc: string, hero: string, active: boolean, comingSoon: boolean, currency: string, destId: string) {
...
function upsertUser(db: Database.Database, id: string, email: string, first: string, last: string, role: "TRAVELLER"|"HOST"|"ADMIN"|"SUPPORT", pwHash: string) {
...
// ---------------------------------------------------------------------------
// Helpers for auto-seed detection
// ---------------------------------------------------------------------------
export function isDatabaseEmpty(db: Database.Database): boolean {
  try {
    const tables = ["properties", "collections", "users", "destinations", "countries"];
    for (const t of tables) {
      const row = db.prepare(`SELECT COUNT(*) as c FROM ${t}`).get() as { c: number };
      if (!row || row.c === 0) return true;
    }
    // Also check if any of the core tables has zero
...
// ---------------------------------------------------------------------------
// Main seed function - idempotent, reuses same data as original script
// ---------------------------------------------------------------------------
export function seedDatabase(db: Database.Database): void {
  ...
  const indoId = upsertCountry(db, cuid(), "Indonesia", "indonesia", "ID");
  const baliId = upsertDestination(db, cuid(), "Bali", "bali",
  ...
  for (const r of [{name:"Uluwatu",slug:"uluwatu"},{name:"Ubud",slug:"ubud"},{name:"Canggu",slug:"canggu"},{name:"Seminyak",slug:"seminyak"},{name:"Nusa Dua",slug:"nusa-dua"}]) {
    regions[r.slug] = upsertRegion(db, cuid(), r.name, r.slug, baliId);
  }

  const baliCol = upsertCollection(db, cuid(), "The Bali Collection", "bali", "Ten handpicked private villas",
  ...
  for (const f of future) {
    const cid = upsertCountry(db, cuid(), f.country, f.slug, f.iso);
    const did = upsertDestination(db, cuid(), f.destination, f.slug, "Coming soon.", U("1519046904884-53103b34b206",2000), f.lat, f.lng, cid);
    upsertCollection(db, cuid(), `The ${f.destination} Collection`, f.slug, "Coming soon",
  ...
  const adminId = upsertUser(db, cuid(), "admin@nexmansion.com", "NexMansion", "Admin", "ADMIN", pwHash);
  const hostUserId = upsertUser(db, cuid(), "host@nexmansion.com", "Made", "Surya", "HOST", pwHash);
  const travellerId = upsertUser(db, cuid(), "traveller@nexmansion.com", "Elena", "Moreau", "TRAVELLER", pwHash);
  ...
  if (firstVilla) {
    ...
    if (!ex) {
      ...
      db.prepare(`INSERT INTO payments (id,booking_id,amount_minor,currency,provider,status,provider_session_id,provider_payment_id,metadata,created_at,updated_at) VALUES (?,?,?,?,?,?,?, ?, ?, datetime('now'), datetime('now'))`).run(cuid(), bid, total, "EUR", "MANUAL", "SUCCEEDED", null, null, null);
    }
  }
}
